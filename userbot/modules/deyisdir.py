...
async def degistir(event):
    ...
    plugin = plugin.strip()
    TURLER = ["afk", "alive", "pm", "kickme", "dızcı", "ban", "mute", "approve", "disapprove", "block", "emoji", "restart", "tagstop"]
    if type(mesaj) is list:
        if plugin in TURLER:
            ...
            await event.edit(LANG['NOT_FOUND'] + ": `afk, alive, emoji, pm, restart, kickme, dızcı, ban, mute, approve, disapprove, block, tagstop`")
    elif len(plugin) < 1:
...
**Kickme Dəyişkənləri:** `{title}`, `{myfirstname}`, `{myfullname}`, `{myusername}`, `{mymention}`'
).add()
